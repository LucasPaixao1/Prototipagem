...
public class ClienteDAO {
	...
	public void cadastraCliente(ClienteVO cVO) throws Exception {
		PreparedStatement preparedStatement = this.connection.prepareStatement(
				"INSERT INTO Cliente(Codigo_Cliente,Nome_Cliente,CNPJ_Cliente,CPF_Cliente,Forma_De_Pagamento_Cliente,Email_Cliente) VALUES(?,?,?,?,?,?)");
		PreparedStatement preparedStatement2 = this.connection.prepareStatement(
				"INSERT INTO Contato(Cel_Contato,Fixo_Contato,Nextel_Contato, Codigo_Cliente) VALUES(?,?,?,?)");
		PreparedStatement preparedStatement1 = this.connection.prepareStatement(
				"INSERT INTO Endereco(Rua_Endereco,Bairro_Endereco,Numero_Endereco,Cep_Endereco,Cidade_Endereco,Complemento_Endereco, Codigo_Cliente)VALUES(?,?,?,?,?,?,?)");
		...
		preparedStatement.setString(6, cVO.getEmail());
		...
		preparedStatement1.setString(6, cVO.getComplemento());
		preparedStatement1.setInt(7, cVO.getCodigo());
	...
	public String excluirCliente(int codigo) throws SQLException {
		...
		PreparedStatement preparedStatement1 = this.connection
				.prepareStatement("DELETE FROM Endereco WHERE Codigo_Cliente = ?");
		PreparedStatement preparedStatement2 = this.connection
				.prepareStatement("DELETE FROM Contato WHERE Codigo_Cliente = ?");
	...
	public ClienteVO pesquisarContatoTodos(int codigo) throws SQLException {
		...
		PreparedStatement preparedStatement2 = this.connection
				.prepareStatement("Select * From Endereco Where Codigo_Cliente = ?");
		PreparedStatement preparedStatement3 = this.connection
				.prepareStatement("Select * From Contato Where Codigo_Cliente = ?");
		preparedStatement1.setInt(1, codigo);
		preparedStatement2.setInt(1, codigo);
		preparedStatement3.setInt(1, codigo);
		...
		if (resultSet1.next() && resultSet2.next() && resultSet3.next()) {
			...
			clienteVO.setEmail(resultSet1.getString("Email_Cliente"));
			clienteVO.setComplemento(resultSet2.getString("Complemento_Endereco"));
	...
	public List<ClienteVO> pesquisarNome(String nome) throws Exception {
		List<ClienteVO> retorno = new ArrayList<ClienteVO>();
		int cont = 1;
		PreparedStatement preparedStatement = this.connection.prepareStatement("Select * from Cliente  where Nome_Cliente like '%" + nome + "%'");
